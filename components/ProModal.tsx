...
export default function ProModal({ isOpen, onClose, shifts, settings, result }: ProModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const payload = {
      ...
      message: "Waitlist signup from overtidskalkulator.no"
    };
        ...
        {isSuccess ? (
          ...
          <>
            <form onSubmit={handleSubmit} className="space-y-3 mb-4">
              ...
              <button
                className="w-full rounded-xl border px-4 py-2 font-medium bg-black text-white hover:bg-black/90 disabled:opacity-60 disabled:cursor-not-allowed"
                type="submit"
                disabled={isSubmitting}
              >
